...
class KickoffPlot():
    ...
    def output_plot(series: Series, save_points: bool) -> None:
        KickoffPlot.plot_first_touches(series=series, save_points=save_points)
        KickoffPlot.plot_winners()
    ...
    @staticmethod
    def determine_winners(event: Kickoff):
        """
    ...
    @staticmethod
    def save_points(filename: str, points) -> None:
        with open(filename, mode="w") as f:
            for point in points:
                f.write(f"{str(point)}\n")

    @staticmethod
    def plot_winners(series: Series, save_points: bool) -> None:
        series_winners = []
        for i, game in enumerate(series.games, 1):
            game_winners = []
            for event in game.events:
                if isinstance(event, Kickoff):
                    first_touch = KickoffPlot.determine_winners(event)
            ...
            plt.savefig(f"{game.path}\\kickoff_winners.png")

            if (save_points):
                save_points(f"{series.series_path}\\game{i}\\{KickoffPlot.get_debug_filename()}", game_winners)

        plt = KickoffPlot._create_plot(series_winners, series)
        plt.savefig(f"{series.series_path}\\kickoff_winners.png")

        if (save_points):
            KickoffPlot.save_points(f"{series.series_path}\\{KickoffPlot.get_debug_filename()}", series_winners)

    @staticmethod
    def plot_first_touches(series: Series, save_points: bool) -> None:
        series_winners = []
        for i, game in enumerate(series.games, 1):
            game_winners = []
            for event in game.events:
